...
void* most_freq_char_countChars(void* args) {
  ...
  for (int i = 0; i < charSubArrayLength; i++) {
    ...
    _Atomic(count_record_count_t*) record = commonCountRecord + scannedChar;
    __atomic_add_fetch(record, 1, __ATOMIC_ACQ_REL);
  }
  return NULL;
}
